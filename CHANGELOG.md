...
### Fixed
- Proof reconstruction for decomposed internal nodes: `_extract_proof_body_ast_guided` no longer passes the full `formal_theorem` (including `:=` and proof body) to `extract_proof_body_from_ast`, which expects only the declaration signature. Target signature is now computed from the parsed AST via `extract_signature_from_ast`, fixing `ProofReconstructionError` when reconstructing subgoals (e.g. `numbertheory_4x3m7y3neq2003` hZMod).
- Tactic body extraction: preserve first-line indentation in `_extract_tactics_from_proof_node`. Replaced `.strip()` with `lstrip("\n").rstrip()` so that when Kimina's `by` token has no leading indent, the first line's leading spaces are not dropped while later lines remain indented. This fixes broken relative indentation in proofs with multiple aligned top-level tactics (e.g. multiple `have ... := by` blocks) and prevents "All indentation strategies failed" for holes such as hlogz (`aime_1983_p1`).

### Added
- Regression test for first-line indent preservation in tactic body extraction.
