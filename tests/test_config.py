...
def test_config_getint_from_ini():
    ...
    max_retries_server = config.getint(section="KIMINA_LEAN_SERVER", option="max_retries")
    assert max_retries_server == 5
...
def test_config_env_override_int(monkeypatch):
    ...
    # Set environment variable
    monkeypatch.setenv("PROVER_AGENT_LLM__MAX_RETRIES", "8192")

    # Value should come from environment variable
    max_retries = config.getint(section="PROVER_AGENT_LLM", option="max_retries")
    assert max_retries == 8192
...
def test_config_multiple_env_overrides(monkeypatch):
    ...
    monkeypatch.setenv("PROVER_AGENT_LLM__MAX_RETRIES", "16384")
    ...
    assert config.getint(section="PROVER_AGENT_LLM", option="max_retries") == 16384
